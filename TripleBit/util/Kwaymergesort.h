...
public:
	...
	KwayMergeSort(const string &inFile, ostream *out,
			...
			int maxBufferSize = 1000000, bool compressOutput = false,
			string tempPath = "./");

	// constructor, using T's overloaded < operator.  Must be defined.
	KwayMergeSort(const string &inFile, ostream *out, int maxBufferSize =
			1000000, bool compressOutput = false, string tempPath = "./");
...
// constructor
template<class T>
KwayMergeSort<T>::KwayMergeSort(const string &inFile, ostream *out,
		bool (*compareFunction)(const T &a, const T &b), int maxBufferSize,
		bool compressOutput, string tempPath) :
		_inFile(inFile), _out(out), _compareFunction(compareFunction), _tempPath(
				tempPath), _maxBufferSize(maxBufferSize), _runCounter(1), _compressOutput(
				compressOutput) {
}
...
template<class T>
KwayMergeSort<T>::KwayMergeSort(const string &inFile, ostream *out,
		int maxBufferSize, bool compressOutput, string tempPath) :
		_inFile(inFile), _out(out), _compareFunction(NULL), _tempPath(tempPath), _maxBufferSize(
				maxBufferSize), _runCounter(0), _compressOutput(compressOutput) {
}
...
template<class T>
void KwayMergeSort<T>::SetComparison(
...
void KwayMergeSort<T>::DivideAndSort() {
	...
	if (input->good() == false) {
		cout << "Error: The requested input file (" << _inFile
				<< ") could not be opened. Exiting!" << endl;
		exit(1);
	}
	vector<T> lineBuffer;
	lineBuffer.reserve(1000000);
	...
	T line;
	/* while (*input >> line) {//*input>>file 遇到空格就算读了一次所以有问题
	...
	//cout<<"maxBuffersize is "<<_maxBufferSize<<endl;
	while (getline(*input, line)) {    //*input>>file 遇到空格就算读了一次所以有问题
		...
		if (totalBytes > _maxBufferSize) {
			...
			// write the sorted data to a temp file
			WriteToTempFile(lineBuffer);
			//cout << _vTempFileNames.size() << endl;
			// clear the buffer for the next run
			lineBuffer.clear();
			_tempFileUsed = true;
			totalBytes = 0;
		...
		// add the current line to the buffer and track the memory used.
		lineBuffer.push_back(line);
		//cout<<"sizeof line is"<<sizeof(line)<<endl;
		totalBytes += sizeof(line);  // buggy?采用sizeof是有问题的
	}
	// handle the run (if any) from the last chunk of the input file.
	if (lineBuffer.empty() == false) {  //最后一部分
		...
		// so we can just dump to the output.文件没有超过指定大小，直接排序后输出
		else {
			if (_compareFunction != NULL)
				sort(lineBuffer.begin(), lineBuffer.end(), *_compareFunction);
			else
...
void KwayMergeSort<T>::WriteToTempFile(const vector<T> &lineBuffer) {
	...
	if (_tempPath.size() == 0)
		...
		tempFileSS << _tempPath << "/" << stl_basename(_inFile) << "."
				<< _runCounter;
...
void KwayMergeSort<T>::Merge() {
	...
	while (outQueue.empty() == false) {
		...
		*_out << lowest.data << endl;//可改进，一条数据用于去更新字典
...
bool isGzipFile(const string& filename) {
	...
	if (gzip_header.id1 == 0x1f && gzip_header.id2 == 0x8b
			&& gzip_header.cm == 8)
		return true;
