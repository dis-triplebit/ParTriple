...
public:
	...
	virtual Status addStatis_new(unsigned v1, unsigned v2, unsigned v3, ID min,
			ID max) = 0;
	/// get a statistics record;
	virtual Status getStatis(unsigned& v1, unsigned v2) = 0;
	virtual Status getStatis_new(unsigned& v1, unsigned &min,unsigned &max,unsigned v2) = 0;
...
public:
	...
	Status addStatis_new(unsigned v1, unsigned v2, unsigned v3, ID min,ID max) {
		//不会被调用
		return addStatis(v1, v2,v3);
		return OK;
	}
	Status getStatis(unsigned& v1, unsigned v2 = 0);
	Status getStatis_new(unsigned& v1, unsigned &min,unsigned &max,unsigned v2 = 0)
	{
		return getStatis(v1, v2);
	}
	Status save(MMapBuffer*& indexBuffer);
	static OneConstantStatisticsBuffer* load(StatisticsType type,
			const string path, char*& indexBuffer);
...
private:
	...
	const unsigned char* decode(const unsigned char* begin,
			const unsigned char* end);
	const unsigned char* decodeID(const unsigned char* begin,
			const unsigned char* end);
	Status decodeAndInsertID(const unsigned char* begin,
			const unsigned char* end, EntityIDBuffer *entBuffer);
...
public:
	...
	Status addStatis_new(unsigned v1, unsigned v2, unsigned v3, ID min,
			ID max) ;
	Status addStatis_newold(unsigned v1, unsigned v2, unsigned v3, ID min,
				ID max) ;
	/// get a statistics record;
	Status getStatis(unsigned& v1, unsigned v2);
	Status getStatis_new(unsigned& v1,unsigned &min,unsigned &max, unsigned v2);
	/// get the buffer position by a id, used in query
	Status getPredicatesByID(unsigned id, EntityIDBuffer* buffer, ID minID,
			ID maxID);
	/// save the statistics buffer;
	Status save(MMapBuffer*& indexBuffer);
	/// load the statistics buffer;
	static TwoConstantStatisticsBuffer* load(StatisticsType type,
			const string path, char*& indxBuffer);
	void flush();
private:
	/// get the value length in bytes;
	unsigned getLen(unsigned v);
	...
	const uchar* decode_new(const uchar* begin, const uchar* end);
	const uchar* decode_new_old(const uchar* begin, const uchar* end);
	...
	const uchar* decode(const uchar* begin, const uchar* end, Triple*, Triple*&,
			Triple*&);
	const uchar* decode_new(const uchar* begin, const uchar* end, Triple*, Triple*&,
				Triple*&);
